...
- **Integración cripto:** **wallet de la app no-custodial, una por usuario**, creada al terminar el registro **para todos** (correo o wallet externa); **login ≠ wallet de la app** (la externa solo identifica). Se muestra en **perfil** y **métodos de pago** (dirección + saldo + exportar llave), **billetera en el dashboard**, **depósitos/retiros/transferencias**, multifirma/modo espejo; todo primero en **DevNet**. La wallet de la app es el **método de pago predefinido** de cada usuario. El flujo de pagos cripto en el detalle del san (pestaña Pagos) hoy solo muestra el monto y que va por la wallet; el pago real va en esta fase.
...
- **Wallet de la app (no-custodial), una por usuario:** al **terminar el registro** —da igual si entró con **correo** o con **wallet externa** (Phantom/Solflare)— la app crea automáticamente **su propia** wallet de Solana **sin custodiar las llaves** (MPC/TEE o llave cifrada con un secreto del usuario; ni la app ni el proveedor tienen la llave completa). **Cada usuario tiene su wallet y su llave secreta; nunca una sola para toda la app.** Se muestra en el **perfil** y en **métodos de pago** (dirección + saldo); la llave es **exportable** (difuminada, con ícono para revelarla, **bajo responsabilidad del usuario**). **Login ≠ wallet de la app:** la wallet externa de login solo prueba identidad; la wallet de ahorro/cobro es la **interna que genera Green Sol**. Sirve de **puente al ahorro en USDC** (estable) para el usuario fiat. Detalle en [SEGURIDAD_Y_WALLETS.md](SEGURIDAD_Y_WALLETS.md).
