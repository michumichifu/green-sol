...
## 0. Estado del proyecto (sesión 2026-06-01, sobre v0.0.65)

Foto rápida de qué funciona, qué falta y qué se decidió en la última conversación.
**Hito de esta sesión:** KYC propio completo (front + back + seguridad + almacenamiento) y **beta desplegada y en vivo** en `https://greensol.creceideas.com` (Docker en la VPS-2), con la base de datos local migrada. Abierta para alpha/beta testing.

### ✅ Implementado (ya funciona)

- **Autenticación (nuevo modelo):** registro (`correo → OTP de verificación → PIN → datos`); login con **PIN de 6 dígitos** como credencial de acceso. Migración automática para cuentas antiguas con contraseña (pantalla "Crea tu PIN"). La **contraseña** se conserva como **factor fuerte** reservado para operaciones cripto. **OTP por correo** disponible como verificación de acciones sensibles (no como credencial de login). Sesión por cookie.
...
- **KYC propio (manual, sin terceros) — completo de punta a punta:**
  ...
  - **Indicadores**: tag **"Verificado"** verde junto al nombre en el perfil cuando `nivelKyc≥1`, y tag **"Sin verificar"** ámbar (enlazado a la verificación) cuando no. Contador `/3` en la sección Verificación.
...
- ~~**KYC manual propio**~~ → **✅ Implementado y probado** (ver sección Hecho). **Decisión cerrada:** el usuario, al terminar el video, ve un **fotograma con un check de confirmación** (no un reproductor) — el super-admin sí reproduce el video completo en su cola. **Pendientes derivados del KYC:**
  ...
  - A futuro: **proveedor externo** (Sumsub/Veriff/MetaMap) + **detección automática de liveness** (anti-spoofing por ML), dejando lo manual como respaldo.
- **Modal de verificación con jerarquía** (estilo Binance — funcionalidad, estética nuestra): al confirmar una acción sensible, pedir **solo el factor más fuerte** que el usuario tenga activo (**biometría > authenticator > email > PIN**). Enganchar a **crear/publicar san** y **métodos de pago**. (El KYC ya estrenó la idea: aprobar/rechazar pide **PIN o contraseña**.)
- **Más factores 2FA:** **TOTP** (Google Authenticator: otplib + QR), **biometría** (WebAuthn/passkeys) y **teléfono** (SMS / WhatsApp) como dato y como 2FA.
- **Flujo de cambio de contraseña** completo: alerta **"Restricciones de cuenta"** (Cancelar/Continuar) + **bloqueo de 24 h** de retiros/P2P/pagos y de unirse/crear ahorros, y pedir el factor más fuerte antes de cambiar.
...
- **Barra de progreso de nivel en el dashboard** (idea tomada de Cashea y confirmada al ver la demo de Noah AI): debajo del nombre del usuario en el hero, una **barra visual** con el **% hacia el siguiente nivel** y cuántos **puntos faltan**. Lo visual engancha a quien no quiere ver números. Hoy el progreso solo está en `/recompensa`; llevarlo al dashboard.
- **Capa cripto / wallets de Solana — la hacemos nosotros** (DevNet). Se probó **Noah AI** (patrocinador) para generar esa capa de referencia: su demo mostró pantalla de wallet (SOL/USDC, depositar/enviar, conectar Phantom/Solflare, wallet embebida no-custodial con frase semilla, airdrop devnet, dirección + explorer), pero **su plan gratuito no permite descargar el código**, así que **no sirve como referencia de código**. Construiremos la conexión de wallets, wallet embebida, transacciones en devnet, firma/confirmación y bote multifirma desde cero en nuestro stack.

> **Próximo foco acordado:** revisar y **mejorar el flujo del san** de punta a punta, y luego la **integración cripto** (DevNet) y los factores de **seguridad** restantes.
>
> **QA pendiente del usuario:** verificar visualmente el **editor de plantillas**, la pantalla **Seguridad** (PIN como credencial / OTP como verificación de acciones / contraseña como factor fuerte), la pantalla **Verificación** (checklist de 2 pasos), y el **flujo completo de crear y finalizar un san**. (El **KYC** ya quedó verificado de punta a punta en la beta en esta sesión.)
...
**Registro y login:**
- **Registro:** flujo `correo → OTP de verificación → PIN (6 dígitos) → datos del perfil`. El **PIN** se crea durante el registro y es la credencial de acceso desde el primer inicio de sesión. La **contraseña** es un factor fuerte opcional configurado en Ajustes → Seguridad, reservado para operaciones de alto valor (retiros cripto). **Migración:** cuentas antiguas con contraseña pero sin PIN son redirigidas a "Crea tu PIN" al intentar entrar.
- **OTP por correo:** código de un solo uso que confirma el correo al registrarse y que, opcionalmente, puede activarse como verificación de acciones sensibles. Sale por el **servidor SMTP propio**. TOTP/Google Authenticator queda para más adelante.
...
- **Seguridad:** tres secciones — **"Acceso a la cuenta"** (PIN, funcional: cambiar/quitar), **"Verificación de acciones"** (OTP por correo con toggle, biometría y TOTP como "Pronto"), **"Factor fuerte"** (contraseña para cripto, funcional: cambiar). Ver §0 Estado del proyecto para el modelo completo.
