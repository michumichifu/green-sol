...
- ~~**KYC manual propio**~~ → **✅ Implementado y probado** (ver sección Hecho). **Pendientes derivados del KYC:**
  - **Reproducción del video para el usuario** (hoy solo ve el fotograma de confirmación): si se quisiera que el usuario lo reproduzca, habría que **convertir el formato** (webm sin duración → mp4, o reescribir la duración con `ts-ebml`). No urgente; el super-admin sí lo reproduce.
